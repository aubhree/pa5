...
public class SchedulingAlgorithmPreemptiveShortestCPUBurstFirst implements SchedulingAlgorithm {
	...
	ArrayList<SimulatedProcess> completedList = new ArrayList<SimulatedProcess>();
			...
			if (list.size() >= 1){
				...
				for(int i = 0; i < list.size(); i++){
					...
					for(int j = 1; j < list.size()-i; j++){
						...
						if(CPUBurst(list.get(j-1)) > CPUBurst(list.get(j))){
	...
	public long CPUBurst(SimulatedProcess process) {
		
		//long arrivalDate = SchedulingMechanisms.getProcessArrivalDate(process);
		//long currentDate = SchedulingMechanisms.getDate();
		long CPUBurst = SchedulingMechanisms.getProcessCPUBurstDuration(process);
		//long currentCPUBurst = (CPUBurst - arrivalDate);
